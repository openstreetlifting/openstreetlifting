...
for tool in docker cargo pnpm; do
  command -v "$tool" >/dev/null || { echo "$tool was not found in PATH. Install it before starting the local servers."; exit 1; }
done
...
if command -v cargo-watch >/dev/null; then
  ...
  echo "Backend reload is disabled. To enable it, run: cargo install cargo-watch"
